...
// Fetch all categories for foods and categories pages
$categories = [];
$categories_data = $conn->query("SELECT * FROM categories");
while ($cat = $categories_data->fetch_assoc()) {
    $categories[$cat['id']] = $cat['name_' . $_SESSION['lang']] ?? $cat['name_en'];
}
$categories_data->data_seek(0);
...
<body class="admin-body <?php echo $theme; ?>">
    ...
    <main class="admin-content">
        ...
        if (file_exists($page_file)) {
            ...
            echo "<div class='admin-section'><p>" . ($lang['page_not_found'] ?? 'Page not found.') . "</p></div>";
        }
        ?>
    </main>
