...
// Calculate totals for daily expenses only
$current_month = date('Y-m');
$last_month = date('Y-m', strtotime('first day of last month'));

$current_total = $conn->query("SELECT SUM(amount) as total FROM expenses WHERE DATE_FORMAT(date, '%Y-%m') = '$current_month' AND type = 'daily'")->fetch_assoc()['total'] ?? 0;
$last_total = $conn->query("SELECT SUM(amount) as total FROM expenses WHERE DATE_FORMAT(date, '%Y-%m') = '$last_month' AND type = 'daily'")->fetch_assoc()['total'] ?? 0;
?>
...
<head>
    ...
    <title>مدیریت هزینه‌های منزل</title>
...
<div class="container">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">مدیریت هزینه‌های منزل</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div class="card">
            <h3 class="text-lg font-semibold text-gray-700">جمع هزینه‌های جاری این ماه</h3>
            <p class="text-2xl text-green-600"><?php echo number_format($current_total, 3); ?> ریال عمان</p>
        </div>
        <div class="card">
            <h3 class="text-lg font-semibold text-gray-700">جمع هزینه‌های جاری ماه قبل</h3>
    ...
    <!-- Expenses Table -->
    <div class="card">
        <h3 class="text-lg font-semibold mb-4">لیست هزینه‌های جاری</h3>
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody>
                    <?php
                    $result = $conn->query("SELECT e.*, p.name as payer_name FROM expenses e LEFT JOIN payers p ON e.payer_id = p.id WHERE e.type = 'daily' ORDER BY e.date DESC");
