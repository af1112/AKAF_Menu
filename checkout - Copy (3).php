...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order'])) {
    ...
    if ($delivery_type === 'dine-in') {
		...
		$latitude = $_POST['latitude-delivery'] ?? ''; // استفاده از latitude-delivery
		$longitude = $_POST['longitude-delivery'] ?? ''; // استفاده از longitude-delivery
		...
		if (empty($address) || empty($contact_number) || empty($recipient_name) || empty($payment_method)) {
			...
			$location = "$latitude,$longitude"; // ذخیره مختصات به صورت رشته
        ...
        if (empty($contact_number)) {
            $error = "A contact number is required to call you in case the food is ready.";
        } else {
        ...
        if (empty($car_brand_name) || empty($car_color) || empty($car_tag_number)) {
            $error = "Car Details are required to deliver your order.";
        } else {
		...
		$address = $_POST['address'] ?? '';
		$latitude = $_POST['latitude-contactless'] ?? ''; // استفاده از latitude-contactless
		$longitude = $_POST['longitude-contactless'] ?? ''; // استفاده از longitude-contactless
		$preferred_time = $_POST['preferred_time'] ?? '';
		$payment_method = $_POST['payment_method'] ?? '';
            ...
            <?php else: ?>
                ...
                <?php if ($order_status === 'Pending'): ?>
                    <form method="POST" class="mt-3">
						...
						<select name="delivery_type" class="delivery-select" required>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!select || !dineInFields || !deliveryFields || !takeawayFields || !drivethruFields || !contactlessFields || !curbsideFields) {
        console.error('One or more elements not found:', {
            select, dineInFields, deliveryFields, takeawayFields, drivethruFields, contactlessFields, curbsideFields
        });
        return;
    }
    ...
    function initMap(containerId) {
        ...
        // مطمئن شو که المان قابل دیدنه
        mapContainer.style.display = 'block';
        mapContainer.style.height = '400px';
        mapContainer.style.width = '100%';

        // چک کن که ابعاد درست باشه
        ...
        // پاک کردن هر گونه نقشه قبلی روی این المان
        if (mapContainer._leaflet_id) {
            console.log(`Removing existing map on ${containerId}`);
            try {
                // پیدا کردن نمونه‌های نقشه روی المان
                ...
                // پاک کردن _leaflet_id برای جلوگیری از تداخل
    ...
    function toggleMap(value) {
        ...
        if (value === 'delivery') {
            deliveryFields.classList.add('show-map');
            contactlessFields.classList.remove('show-map');
            clearMap();
            setTimeout(() => {
                if (!isMapInitializing) {
                    isMapInitializing = true;
